...
Mixarr v2.4.0 adds optional provider-neutral infrastructure for Ollama, LiteLLM, LM Studio, DeepSeek, OpenAI API, generic OpenAI-compatible services, OpenRouter, and native Anthropic. v2.4.1 adds privacy, pricing, spending, token, request-volume, background, retry, reservation, alert, and usage controls. v2.4.2 adds the review-first Ask Mixarr workflow at `/ask-mixarr`; later v2.4.x releases add Recipe Copilot, advisory summaries and metadata suggestions, explanations, troubleshooting, and hardened governance. v2.4.10 brings those systems together at `/ai` with resumable setup, history, saved request templates, quality feedback, and operational summaries. Configure advanced provider and policy details at `/settings/ai`. AI provider requests default to 120 seconds and may be capped deployment-wide with `AI_REQUEST_TIMEOUT_SECONDS` (30–600); the stricter global, provider, request, or governance timeout still wins. ChatGPT consumer subscriptions are not OpenAI API credentials and are not integrated through browser sessions or unofficial tokens.
