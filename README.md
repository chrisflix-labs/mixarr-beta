...
Mixarr v2.4.0 adds optional provider-neutral infrastructure for Ollama, LiteLLM, LM Studio, DeepSeek, OpenAI API, generic OpenAI-compatible services, OpenRouter, and native Anthropic. v2.4.1 adds privacy, pricing, spending, token, request-volume, background, retry, reservation, alert, and usage controls. v2.4.2 adds the review-first Ask Mixarr workflow at `/ask-mixarr`; later v2.4.x releases add Recipe Copilot, advisory summaries and metadata suggestions, explanations, troubleshooting, and hardened governance. v2.4.10 brings those systems together at `/ai` with resumable setup, history, saved request templates, quality feedback, and operational summaries. Configure advanced provider and policy details at `/settings/ai`. In v2.4.22, connection, first-token, total-request, and streaming-idle phases can each be finite or Unlimited. Enabled provider policies replace the global policy and can relax it for local models; cancellation and worker heartbeats remain active. ChatGPT consumer subscriptions are not OpenAI API credentials and are not integrated through browser sessions or unofficial tokens.

AI is globally disabled after upgrade, and Mixarr makes no provider request until an administrator explicitly approves a provider, model, feature, privacy mode, and any external data categories. Non-administrators have no AI permissions until explicitly granted. External providers, payload retention, paid fallback, external background work, and secure payload recording are off by default; unknown fields and unpriced external models are blocked. Set `AI_CREDENTIAL_ENCRYPTION_KEY` (or `MIXARR_SECRET_KEY`) before saving secret-based providers, and set `MIXARR_AI_ENABLED=false` for a deployment-level shutdown that the UI cannot override. See [AI Governance, Security and Reliability](docs/AI_GOVERNANCE_SECURITY_RELIABILITY_V249.md), [AI Provider Foundation](docs/AI_PROVIDER_FOUNDATION_V240.md), and [AI Privacy and Cost Controls](docs/AI_GOVERNANCE_V241.md).
