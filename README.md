...
Mixarr v2.4.0 adds optional provider-neutral infrastructure for Ollama, LiteLLM, LM Studio, DeepSeek, OpenAI API, generic OpenAI-compatible services, OpenRouter, and native Anthropic. v2.4.1 adds privacy, pricing, spending, token, request-volume, background, retry, reservation, alert, and usage controls. v2.4.2 adds the review-first Ask Mixarr workflow at `/ask-mixarr` and the Ollama Requests & User Policy hotfix. Configure providers and explicitly enable the feature at `/settings/ai`. ChatGPT consumer subscriptions are not OpenAI API credentials and are not integrated through browser sessions or unofficial tokens.
