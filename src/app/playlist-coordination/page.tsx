...
import CrossPlaylistVarietyWorkspace from "@/components/CrossPlaylistVarietyWorkspace";
...
type Settings = { coordinationEnabled: boolean; maximumSharedTrackPercentage: number; overlapEnforcement: string; keepDistinct: boolean; allowSharedCoreTracks: boolean; maximumSharedCoreTracks?: number | null; preferGloballyUnusedTracks: boolean; unusedTrackPreferenceStrength: number; maximumCoordinationInfluence: number; crossPlaylistArtistBalancingEnabled: boolean; maximumSharedArtistPercentage?: number | null; maximumSharedAlbumPercentage?: number | null; maximumSharedTrackCount?: number | null; minimumUniqueTrackPercentage: number; minimumUniqueTrackCount?: number | null; uniqueTargetMode: string; recentUsageLookbackDays?: number | null; recentUsagePenaltyStrength: string; sharedTrackAllowance: number; coreTrackAllowance?: number | null; comparisonScope: string; comparisonGroupIds: string[]; automaticRepairEnabled: boolean; requireRepairPreview: boolean; excludedFromEnforcement: boolean; exclusivityBehavior: string; exclusivityLookbackDays?: number | null; ignoreManualAdditionsForExclusivity: boolean; maximumTracksPerArtistAcrossGroup?: number | null; featuredArtistMatching: string; warnBeforeExceedingOverlap: boolean; excludedPlaylistIds: string[] };

const defaultSettings: Settings = { coordinationEnabled: false, maximumSharedTrackPercentage: 20, overlapEnforcement: "SOFT_TARGET", keepDistinct: false, allowSharedCoreTracks: false, maximumSharedCoreTracks: null, preferGloballyUnusedTracks: false, unusedTrackPreferenceStrength: 0.5, maximumCoordinationInfluence: 12, crossPlaylistArtistBalancingEnabled: true, maximumSharedArtistPercentage: 35, maximumSharedAlbumPercentage: 25, maximumSharedTrackCount: null, minimumUniqueTrackPercentage: 70, minimumUniqueTrackCount: null, uniqueTargetMode: "PREFERRED", recentUsageLookbackDays: 30, recentUsagePenaltyStrength: "MEDIUM", sharedTrackAllowance: 0, coreTrackAllowance: null, comparisonScope: "ALL_MANAGED", comparisonGroupIds: [], automaticRepairEnabled: false, requireRepairPreview: true, excludedFromEnforcement: false, exclusivityBehavior: "OFF", exclusivityLookbackDays: null, ignoreManualAdditionsForExclusivity: true, maximumTracksPerArtistAcrossGroup: 6, featuredArtistMatching: "PRIMARY_ONLY", warnBeforeExceedingOverlap: true, excludedPlaylistIds: [] };
    ...
    <main className={styles.page}>
      ...
      <section className={styles.panel}><div className={styles.panelHeader}><div><h3>Playlist settings</h3><p>Coordination is conservative and disabled for legacy playlists until you enable it.</p></div><select value={settingsPlaylistId} onChange={(event) => setSettingsPlaylistId(event.target.value)}>{playlists.map((playlist) => <option key={playlist.id} value={playlist.id}>{playlist.plexPlaylistTitle}</option>)}</select></div>
        <div className={styles.settingsGrid}>
          ...
          <label>Maximum shared artists <strong>{settings.maximumSharedArtistPercentage ?? 35}%</strong><input type="range" min="0" max="100" value={settings.maximumSharedArtistPercentage ?? 35} onChange={(event) => setSettings({ ...settings, maximumSharedArtistPercentage: Number(event.target.value) })} /></label>
          <label>Maximum shared albums <strong>{settings.maximumSharedAlbumPercentage ?? 25}%</strong><input type="range" min="0" max="100" value={settings.maximumSharedAlbumPercentage ?? 25} onChange={(event) => setSettings({ ...settings, maximumSharedAlbumPercentage: Number(event.target.value) })} /></label>
          <label>Unique-track target <strong>{settings.minimumUniqueTrackPercentage}%</strong><input type="range" min="0" max="100" value={settings.minimumUniqueTrackPercentage} onChange={(event) => setSettings({ ...settings, minimumUniqueTrackPercentage: Number(event.target.value) })} /></label>
          <label>Unique target<select value={settings.uniqueTargetMode} onChange={(event) => setSettings({ ...settings, uniqueTargetMode: event.target.value })}><option value="PREFERRED">Preferred</option><option value="STRICT">Strict with graceful fallback</option></select></label>
          <label>Recent cross-playlist penalty<select value={settings.recentUsagePenaltyStrength} onChange={(event) => setSettings({ ...settings, recentUsagePenaltyStrength: event.target.value })}><option value="OFF">Off</option><option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option><option value="STRICT">Strict</option></select></label>
          <label>Comparison scope<select value={settings.comparisonScope} onChange={(event) => setSettings({ ...settings, comparisonScope: event.target.value })}><option value="ALL_MANAGED">All managed playlists</option><option value="RELATED_ONLY">Related playlists only</option><option value="SIMILAR_IDENTITIES">Similar identities</option><option value="SELECTED_GROUPS">Selected groups</option></select></label>
          <label>Exclusivity behavior<select value={settings.exclusivityBehavior} onChange={(event) => setSettings({ ...settings, exclusivityBehavior: event.target.value })}><option value="OFF">Off</option><option value="PREFER_EXCLUSIVE">Prefer exclusive, allow fallback</option><option value="STRICT_EXCLUSIVE">Strict with graceful fallback</option></select></label>
          ...
          <label className={styles.toggle}><input type="checkbox" checked={settings.excludedFromEnforcement} onChange={(event) => setSettings({ ...settings, excludedFromEnforcement: event.target.checked })} /><span><strong>Report only</strong><small>Keep this playlist in reports but exclude it from variety enforcement.</small></span></label>
          <label className={styles.toggle}><input type="checkbox" checked={settings.automaticRepairEnabled} onChange={(event) => setSettings({ ...settings, automaticRepairEnabled: event.target.checked })} /><span><strong>Automatic repair</strong><small>Disabled by default; preview and approval remain available.</small></span></label>
          <label className={styles.toggle}><input type="checkbox" checked={settings.requireRepairPreview} onChange={(event) => setSettings({ ...settings, requireRepairPreview: event.target.checked })} /><span><strong>Require repair preview</strong><small>Recommended safeguard before any replacement is applied.</small></span></label>
