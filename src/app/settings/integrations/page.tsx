...
export default async function IntegrationsPage() {
  ...
  const [servers, users, integrations, tokens, endpoints, deliveries, tests, mounts, collectionStates] = await Promise.all([
    ...
    db.user.findMany({ select: { id: true, username: true, email: true, _count: { select: { generatedPlaylists: true } }, plexUserMappings: { include: { server: { select: { id: true, name: true } }, plexAccount: { select: { id: true, plexUserId: true, username: true, email: true, accountType: true, isAvailable: true } } } } } }),
  ...
  return <IntegrationCenter initial={{ servers, users, integrations: integrations.map((row: any) => ({ ...row, encryptedSecretJson: undefined, secretsConfigured: !!row.encryptedSecretJson })), tokens, endpoints, deliveries, tests, mounts, collectionStates }} />;
}
