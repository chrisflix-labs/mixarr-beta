...
export default function LibraryHealthPage() {
  ...
  const retryBpm = async (payload: { trackIds?: string[]; filter?: BpmFilter; libraryId?: string; providerMode?: RetryProviderMode }) => {
    ...
    if (data.queued > 0) {
      ...
      setMessage(`${data.message || data.summary || `Queued ${data.queued} track${data.queued === 1 ? "" : "s"} for BPM retry.`}${suffix}`);
    } else {
      setMessage(data.message || data.summary || "No eligible active tracks were queued. Tracks with a valid BPM require a forced retry.");
    }
    await loadSummary();
    ...
    if (data.queued > 0) {
      const startResponse = await fetch("/api/sync/start", {
        ...
        body: JSON.stringify({
          ...
          providerMode,
          libraryId: payload.libraryId,
          audioFeaturePartialBefore: data.before,
          filter: payload.filter,
          trackIds: payload.trackIds,
          force: providerMode === "force_local",
          retry: true,
      ...
      setMessage(`${data.message || data.summary || `Queued ${data.queued} track${data.queued === 1 ? "" : "s"} for audio-feature retry.`}${suffix}`);
    } else {
      setMessage(data.message || data.summary || "No eligible active tracks were queued.");
    }
    await loadSummary();
