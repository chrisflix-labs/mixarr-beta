...
type PlaylistRecipe = {
  ...
  governance?: { trustState: string; approvalState: string; quarantineState: string; official: boolean; compatibilityStatus: string; riskLevel: string; lastValidatedAt?: string | null };
};
...
function RecipePreviewCard({ recipe, technical }: { recipe: PreviewRecipe; technical?: boolean }) {
  return <article className={styles.previewRecipe} data-invalid={!recipe.ready}>
    <header><div><h4>{recipe.name}</h4><p>{recipe.description || "No description provided."}</p></div><span>{recipe.ready ? recipe.adaptations.length ? "Adaptation required" : recipe.conflicts.length ? "Conflict" : "Ready" : "Blocked"}</span></header>
    {recipe.governance && <section className={styles.governanceReview} data-risk={recipe.governance.risk.riskLevel}>
      <h5>{recipe.governance.official ? "Official Recipe" : recipe.governance.trustState.replaceAll("_", " ")} · {recipe.governance.risk.riskLevel} risk ({recipe.governance.risk.score}/100)</h5>
      <p><b>Signature:</b> {recipe.governance.signature.status.replaceAll("_", " ")} · <b>Compatibility:</b> {recipe.governance.compatibility.status.replaceAll("_", " ")} · <b>Recommended:</b> {recipe.governance.recommendedImportMode.replaceAll("_", " ")}</p>
      {recipe.governance.quarantine.required && <div><strong>This recipe will be quarantined.</strong>{recipe.governance.quarantine.reasons.map((reason: string) => <p key={reason}>• {reason}</p>)}</div>}
      <h6>Requested permissions</h6>
      {recipe.governance.permissions.map((item: any) => <p key={item.permission}><b>{item.permission}</b> · {item.riskLevel} · {item.decision}: {item.reason}{item.fallback ? ` Safer fallback: ${item.fallback}.` : ""}</p>)}
      {recipe.governance.risk.findings.map((item: any) => <p className={styles.warningText} key={item.code}>{item.message}</p>)}
      {recipe.governance.safetyAdjustments.length > 0 && <h6>Safety-limit adjustments</h6>}{recipe.governance.safetyAdjustments.map((item: any) => <p className={styles.warningText} key={item.path}>{item.message}</p>)}
    </section>}
    <dl className={styles.recipeSummary}>{Object.entries(recipe.summary).filter(([key]) => key !== "title").map(([key, value]) => <div key={key}><dt>{key.replace(/([A-Z])/g, " $1")}</dt><dd>{value}</dd></div>)}</dl>
    <div className={styles.previewBadges}><span data-ok={recipe.checksumStatus === "valid"}>Checksum: {recipe.checksumStatus}</span><span data-ok={recipe.sensitiveDataScan.safe}>Security: {recipe.sensitiveDataScan.safe ? "passed" : "blocked"}</span><span>{recipe.compatibleSettings} compatible</span><span>{recipe.adaptations.length} adaptations</span><span>{recipe.unsupported.length} unsupported</span></div>
    {recipe.conflicts.map((conflict) => <p className={styles.warningText} key={`${conflict.type}-${conflict.message}`}>{conflict.message}</p>)}
    {recipe.validationErrors.map((item) => <p className={styles.errorText} key={`${item.path}-${item.code}`}>{item.message}</p>)}
    {technical && <details><summary>Technical details</summary><p>Recipe schema v{recipe.sourceRecipeVersion}; export format v{recipe.sourceFormatVersion}; exporting Mixarr {recipe.exportingApplicationVersion || "unknown"}.</p>{recipe.migrationSteps.map((step) => <p key={step}>{step}</p>)}{recipe.compatibility.filter((item) => item.classification !== "compatible").map((item) => <p key={`${item.path}-${item.message}`}>{item.classification}: {item.path}: {item.message}</p>)}{recipe.validationWarnings.map((item) => <p key={`${item.path}-${item.code}`}>{item.path}: {item.message}</p>)}{recipe.adaptations.map((item) => <p key={item.path}>{item.path}: {item.reason} Proposed: {String(item.proposedValue)}</p>)}{recipe.unsupported.map((item) => <p key={item.path}>{item.path}: {item.message}</p>)}</details>}
  </article>;
}

function RecipeGovernanceApproval({ recipe, decision, onChange }: { recipe: PreviewRecipe; decision?: Decision; onChange: (patch: Partial<Decision>) => void }) {
  const governance = recipe.governance;
  if (!governance) return null;
  const granted = decision?.grantedPermissions || [];
  const consequences = decision?.confirmConsequences || [];
  return <fieldset className={styles.modeChoice}>
    <legend>{recipe.name} safety approval</legend>
    <p><b>Import mode</b> — recommended: {governance.recommendedImportMode.replaceAll("_", " ")}</p>
    {governance.availableImportModes.map((value: string) => <label key={value}><input type="radio" checked={decision?.importMode === value} onChange={() => onChange({ importMode: value })} /><span><b>{value.replaceAll("_", " ")}</b><small>{value === "suggest_only" ? "No automatic execution; proposed changes require review." : value === "approval_required" ? "Every generated change requires local approval." : value === "automatic_with_limits" ? "Automation remains inside configured safety limits." : "Use declared settings; administrator permission may be required."}</small></span></label>)}
    <p><b>Requested permissions</b> — optional permissions can be removed before import.</p>
    {governance.permissions.map((item: any) => {
      const checked = granted.includes(item.permission);
      return <div key={item.permission}><label><input type="checkbox" disabled={item.decision === "deny" || (item.required === true && item.decision === "allow")} checked={checked} onChange={(event) => onChange({ grantedPermissions: event.target.checked ? [...granted, item.permission] : granted.filter((value: string) => value !== item.permission) })} /><span><b>{item.permission}</b><small>{item.reason} · {item.required ? "required" : "optional"} · {item.riskLevel} risk · {item.decision}{item.fallback ? ` · ${item.fallback}` : ""}</small></span></label>{item.riskLevel === "high" && checked && <label className={styles.confirmRisk}><input type="checkbox" checked={consequences.includes(item.permission)} onChange={(event) => onChange({ confirmConsequences: event.target.checked ? [...consequences, item.permission] : consequences.filter((value: string) => value !== item.permission) })} /><span><b>Authorize {item.permission}</b><small>I understand: {item.reason} Protected playlists and absolute safety caps remain enforced.</small></span></label>}</div>;
    })}
  </fieldset>;
}
