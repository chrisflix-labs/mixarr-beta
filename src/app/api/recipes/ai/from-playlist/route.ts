...
export async function POST(request: Request) {
  try { const body = await request.json(); return NextResponse.json({ proposal: await runRecipeCopilot(recipeCopilotUserId(), null, { ...body, action: "from_playlist" }, request.signal) }, { status: 201 }); }
