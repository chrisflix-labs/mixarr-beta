...
export async function GET(_: Request, { params }: { params: { id: string } }) { try { const value = await getRecommendationExplanation(recommendationExplanationUserId(), params.id); return NextResponse.json({ generatedSettings: value.generatedSettings, validationResults: value.validationResults, ruleTraces: value.ruleTraces, responsibility: value.responsibility }); } catch (error) { return recommendationExplanationApiError(error); } }
