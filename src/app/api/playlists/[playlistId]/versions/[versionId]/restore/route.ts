...
export async function POST(request: Request, { params }: { params: { playlistId: string; versionId: string } }) {
  ...
  try {
    ...
    return NextResponse.json(await restorePlaylistVersion({ userId, generatedPlaylistId: params.playlistId, versionId: params.versionId, expectedPlaylistUpdatedAt: parsed.data.expectedPlaylistUpdatedAt, missingTrackStrategy: parsed.data.missingTrackStrategy, restoreSettings: parsed.data.restoreSettings, restorePlaylistMetadata: parsed.data.restorePlaylistMetadata }));
  } catch (error) {
