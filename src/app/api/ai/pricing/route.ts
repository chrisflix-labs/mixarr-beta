...
export async function POST(request: Request) { try { const actorId = await requireAiPermission("MANAGE_AI_PRICING"); const body = await request.json(); const providerId = body.importProviderModels || body.providerConfigId; if (providerId) await assertActiveAiProviderId(String(providerId)); return NextResponse.json(body.importProviderModels ? await importDiscoveredPricingProfiles(String(body.importProviderModels), actorId) : await saveAiPricingProfile(body, actorId), { status: 201 }); } catch (error) { return aiRouteError(error); } }
