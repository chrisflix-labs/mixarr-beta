...
export async function PATCH(request: Request, { params }: { params: { providerId: string } }) {
  try {
    ...
    const { modelIdentifier, reason, allowedFeatures, capabilities, approved, ...limits } = tokenLimitSchema.parse(await request.json());
    ...
    const model = await prisma.$transaction(async (tx) => {
      const updated = await tx.aiProviderModel.update({ where: key, data: { ...limits, ...(allowedFeatures ? { allowedFeaturesJson: allowedFeatures } : {}), ...(capabilities ? { capabilitiesJson: capabilities } : {}), ...(approved === undefined ? {} : { approved, approvedBy: approved ? actorId : null, approvedAt: approved ? new Date() : null }) } });
