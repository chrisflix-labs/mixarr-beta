...
export async function POST(request: Request) {
  ...
  try {
    ...
    await safeRecordJobHistory({
      userId,
      type: "library_health",
      name: "Missing track check",
      status: "success",
      trigger: "manual",
      summary: `Missing track check completed. checked=${candidates.length}, restored=${found.length}, stillMissing=${stillMissing}.`,
      counts: { attempted: candidates.length, processed: found.length, skipped: stillMissing, failed: 0 },
    ...
    await safeRecordJobHistory({
      ...
      status: "failed",
      trigger: "manual",
      summary: "Missing track check failed.",
      error,
      metadata: { libraryId: library.id },
    });
