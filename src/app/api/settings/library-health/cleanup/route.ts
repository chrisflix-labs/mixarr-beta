...
export async function POST(request: Request) {
  ...
  if (dryRun) {
    await safeRecordJobHistory({
      userId,
      type: "cleanup",
      name: "Library Health cleanup preview",
      status: "success",
      trigger: "manual",
      summary: `Cleanup preview found ${targets.length} missing tracks eligible for deletion.`,
      counts: { attempted: targets.length, processed: 0, skipped: targets.length, failed: 0 },
      metadata: { libraryId: libraryId || null, days, dryRun: true },
    });
    return NextResponse.json({ count: targets.length, days, cutoff });
  }
  ...
  if (!targets.length) {
    await safeRecordJobHistory({
      userId,
      type: "cleanup",
      name: "Library Health cleanup",
      status: "success",
      trigger: "manual",
      summary: "Cleanup completed. attempted=0, processed=0, skipped=0, failed=0.",
      counts: { attempted: 0, processed: 0, skipped: 0, failed: 0 },
      metadata: { libraryId: libraryId || null, days },
    });
    return NextResponse.json({ deleted: 0, days });
  }
  ...
  await safeRecordJobHistory({
    ...
    status: "success",
    trigger: "manual",
    summary: `Cleanup completed. attempted=${targets.length}, processed=${result}, skipped=${Math.max(0, targets.length - result)}, failed=0.`,
    counts: { attempted: targets.length, processed: result, skipped: Math.max(0, targets.length - result), failed: 0 },
    metadata: { libraryId: libraryId || null, days },
  });
