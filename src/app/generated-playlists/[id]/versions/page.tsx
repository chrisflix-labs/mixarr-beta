...
import { DEFAULT_SCORING_MODEL } from "@/lib/scoringModelCatalog";
...
type VersionSummary = {
  ...
  betaMetadata?: { accessLevel?: string; scoringModel?: string; scoringModelVersion?: string; enabledFeatureFlags?: string[]; stableFallbackUsed?: boolean } | null;
    ...
    {!versions.length ? <section className={styles.empty}><History size={32} /><h3>No playlist history yet</h3><p>Future generations and playlist changes will appear here.</p></section> : <div className={styles.layout}>
      <section className={styles.timeline} aria-label="Playlist versions">
        {versions.map((version) => <button key={version.id} className={`${styles.versionCard} ${selectedId === version.id ? styles.selected : ""}`} onClick={() => setSelectedId(version.id)} aria-label={`Open Version ${version.revisionNumber}${version.isCurrent ? ", current" : ""}`}>
          ...
          <span>{version.label || version.description || reasonLabel(version.reason)} {version.betaMetadata && <b className={styles.betaBadge}>BETA</b>}</span><small><Clock3 size={12} /> {new Date(version.createdAt).toLocaleString()}</small><small>{version.engineVersion ? `Smart Mix Engine ${version.engineVersion}` : "Legacy version"} · {version.betaMetadata?.scoringModel || DEFAULT_SCORING_MODEL} · {version.trackCount} tracks · {duration(version.durationMs)}</small>
        </button>)}
      </section>
      <section className={styles.detail} aria-live="polite">
        {!selected || !detail ? <p>Choose a version to inspect it.</p> : <>
          <div className={styles.detailHeader}><div><span className={styles.kicker}>Version {selected.revisionNumber}{selected.isCurrent ? " · Current" : ""}{selected.betaMetadata ? ` · ${selected.betaMetadata.accessLevel?.replaceAll("_", " ") || "BETA"}` : ""}</span><h3>{selected.label || selected.description || reasonLabel(selected.reason)} {selected.betaMetadata && <b className={styles.betaBadge}>BETA</b>}</h3><p>{new Date(selected.createdAt).toLocaleString()} · {selected.engineVersion || "Legacy engine"} · {selected.betaMetadata?.scoringModel || DEFAULT_SCORING_MODEL} · Mixarr {selected.applicationVersion || "not recorded"}</p>{selected.betaMetadata && <p>Created with: {(selected.betaMetadata.enabledFeatureFlags || []).join(", ") || "stable fallback"}{selected.betaMetadata.stableFallbackUsed ? " · Stable fallback used" : ""}</p>}</div><div className={styles.actions}><button onClick={togglePin} aria-label={`${selected.isPinned ? "Unpin" : "Pin"} Version ${selected.revisionNumber}`}>{selected.isPinned ? <PinOff size={15} /> : <Pin size={15} />}{selected.isPinned ? "Unpin" : "Pin version"}</button>{!selected.isCurrent && <button className={styles.restoreButton} onClick={previewRestore} disabled={Boolean(busy)} aria-label={`Restore playlist Version ${selected.revisionNumber}`}><RotateCcw size={15} /> Restore This Version</button>}</div></div>
