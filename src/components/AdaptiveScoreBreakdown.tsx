...
import { BrainCircuit, Gauge, Headphones, Info } from "lucide-react";
...
export default function AdaptiveScoreBreakdown({ score, playback, defaultOpen = false }: { score?: any; playback?: any; defaultOpen?: boolean }) {
  if (!score && !playback) return null;
  if (score && !score.enabled && !playback?.enabled) {
  ...
  const finalScore = playback?.finalScore ?? adaptiveScore;
