...
export default function BetaAdministration() {
  ...
  const load = async () => {
    ...
    setPayload(features.data); setUsage(usageResponse.data);
  ...
  const updateFeature = async (definition: Definition, patch: Partial<Override>) => {
    setWorking(definition.key); setMessage("");
    ...
    await load(); setWorking(""); setMessage(`${definition.name} updated.`);
  };
  const updateAccess = async (userId: string) => {
    const draft = accessDrafts[userId];
    setWorking(`user:${userId}`); await axios.put("/api/admin/beta/features", { action: "access", userId, accessLevel: draft.accessLevel, expiresAt: draft.expiresAt || null, notes: draft.notes }); await load(); setWorking(""); setMessage("Beta access updated.");
  };
  const copyDiagnostics = async () => {
    await navigator.clipboard.writeText(JSON.stringify({ serverAccessLevel: payload?.serverAccessLevel, overrides: payload?.overrides, usage }, null, 2)); setMessage("Sanitized beta administration summary copied.");
  };
  ...
  return <div className={styles.betaSettings}>
    ...
    <label className={styles.toggleRow}><input type="checkbox" checked={payload.sponsorsCardHidden} onChange={async (event) => { await axios.put("/api/admin/beta/features", { action: "sponsors", hidden: event.target.checked }); await load(); }} /><span><strong>Hide GitHub Sponsors beta card</strong><small>This controls messaging only and never changes access or stable functionality.</small></span></label>

    <div className={styles.betaActions}><button className={styles.secondaryButton} type="button" onClick={() => void load()}><RefreshCw size={15} /> Refresh</button><button className={styles.secondaryButton} type="button" onClick={() => void copyDiagnostics()}><Copy size={15} /> Copy diagnostic information</button>{message && <span><CheckCircle2 size={14} /> {message}</span>}</div>
  </div>;
}
