...
export default function IntegrationCenter({ initial }: { initial: any }) {
  ...
  return <main className={styles.wrapper}>
    ...
    <section id="tokens"><h2>Scoped API tokens</h2><div className={styles.split}><div className={styles.subpanel}><label>Token name<input value={tokenForm.name} onChange={(e) => setTokenForm({ ...tokenForm, name: e.target.value })}/></label><div className={styles.scopes}>{API_TOKEN_SCOPES.map((scope) => <label key={scope}><input type="checkbox" checked={tokenForm.scopes.includes(scope)} onChange={(e) => setTokenForm({ ...tokenForm, scopes: e.target.checked ? [...tokenForm.scopes, scope] : tokenForm.scopes.filter((item) => item !== scope) })}/>{scope}</label>)}</div><button onClick={() => void createToken()} disabled={!!busy || !tokenForm.name || !tokenForm.scopes.length}>Create token</button>{newToken && <div className={styles.tokenReveal}><strong>Copy now — shown once</strong><code>{newToken}</code><button onClick={() => navigator.clipboard.writeText(newToken)}>Copy</button></div>}</div><div className={styles.subpanel}>{data.tokens.map((token: any) => <div className={styles.row} key={token.id}><span><strong>{token.name}</strong><small>{token.prefix}… · last used {date(token.lastUsedAt)} · {(token.scopesJson || []).join(", ")}</small></span>{token.enabled ? <button onClick={() => void revokeToken(token.id)} disabled={!!busy}>Revoke</button> : <em data-tone="muted">Revoked</em>}</div>)}</div></div></section>
