...
export default function BetaFeatureSettingsForm() {
  ...
  return (
    <div className={styles.betaSettings}>
      ...
      {diagnosticReport && <details open><summary>Sanitized diagnostic preview</summary><pre className={styles.betaReportPreview}>{reportText}</pre><div className={styles.betaActions}><button type="button" className={styles.secondaryButton} onClick={() => void copyTextToClipboard(reportText).then(() => setError("")).catch(() => setError("Your browser blocked automatic copying. Use Download Report, or select the report text above and copy it manually."))}><Copy size={15} /> Copy Report</button><button type="button" className={styles.secondaryButton} onClick={downloadDiagnostics}><Download size={15} /> Download Report</button>{payload.support?.discordSupportUrl && <a className={styles.secondaryButton} href={payload.support.discordSupportUrl} target="_blank" rel="noopener noreferrer">Open Discord Support</a>}</div></details>}
