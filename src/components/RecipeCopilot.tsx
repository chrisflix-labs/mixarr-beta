  ...
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !running) onClose(); }}>
    <aside ref={dialog} className={styles.drawer} role="dialog" aria-modal="true" aria-labelledby="recipe-copilot-title" aria-describedby="recipe-copilot-description">
      ...
      <div className={styles.body}>
        ...
        {availability?.available && availability.reasoningConsumesCompletionBudget && <section className={styles.preflight} role="status"><AlertTriangle size={17}/><div><strong>Reasoning output budget</strong><span>This model may use part of the output-token budget for internal reasoning.</span><span>Final-answer target: {Number(availability.finalAnswerTargetTokens || 0).toLocaleString()} · reasoning reserve: {Number(availability.reasoningReserveTokens || 0).toLocaleString()} · recommended minimum: {Number(availability.recommendedMinimumOutputTokens || 0).toLocaleString()}.</span><span>Maximum output: {Number(availability.maximumOutputTokens || 0).toLocaleString()} · model maximum: {availability.modelMaximumOutputTokens ? Number(availability.modelMaximumOutputTokens).toLocaleString() : "not reported"} · estimated maximum cost: {availability.local ? "local policy" : `${availability.currency || "USD"} ${Number(availability.maximumEstimatedCost || 0).toFixed(4)}`}.</span></div></section>}
