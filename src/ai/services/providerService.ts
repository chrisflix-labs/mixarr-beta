...
function validateBaseUrl(value: string | null | undefined) {
  ...
  if (!["http:", "https:"].includes(url.protocol) || url.username || url.password) throw invalidProviderField("baseUrl", "Provider URLs must use HTTP(S) and cannot contain credentials.");
