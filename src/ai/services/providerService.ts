...
import { MAX_AI_TIMEOUT_MS, MAX_CANCELLATION_GRACE_MS, MIN_CANCELLATION_GRACE_MS, resolveEffectiveTimeoutPolicy } from "../config/timeout";
import { cancelAiRequestsForProvider } from "../utilities/cancellation";

const jsonObject = z.record(z.unknown()).default({});
const timeoutValue = z.number().int().positive("Enter a positive whole number, or choose Unlimited.").max(MAX_AI_TIMEOUT_MS).nullable();
export const providerInputSchema = z.object({
  ...
  maximumContextTokens: z.number().int().min(1).max(10_000_000).nullable().optional(),
  requestTimeoutMs: z.number().int().positive().max(MAX_AI_TIMEOUT_MS).optional(),
  timeoutOverrideEnabled: z.boolean().optional(),
  connectionTimeoutMs: timeoutValue.optional(),
  firstTokenTimeoutMs: timeoutValue.optional(),
  totalRequestTimeoutMs: timeoutValue.optional(),
  streamingIdleTimeoutMs: timeoutValue.optional(),
  cancellationGraceMs: z.number().int().min(MIN_CANCELLATION_GRACE_MS).max(MAX_CANCELLATION_GRACE_MS).optional(),
  retryCount: z.number().int().min(0).max(1).optional(), initialRetryDelayMs: z.number().int().min(50).max(60_000).optional(), maximumRetryDelayMs: z.number().int().min(50).max(300_000).optional(), retryBackoffMultiplier: z.number().min(1).max(10).optional(), sslVerification: z.boolean().optional(),
  ...
  requestsPerMinute: z.number().int().positive().max(1_000_000).nullable().optional(), maximumConcurrency: z.number().int().positive().max(1000).nullable().optional(),
}).superRefine((input, context) => {
  if (input.timeoutOverrideEnabled !== true) return;
  for (const field of ["connectionTimeoutMs", "firstTokenTimeoutMs", "totalRequestTimeoutMs", "streamingIdleTimeoutMs", "cancellationGraceMs"] as const) {
    if (!Object.prototype.hasOwnProperty.call(input, field)) context.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: "Choose a value before enabling the provider timeout override." });
  }
});
...
function withEffectiveTimeoutPolicy(provider: any, governance: any) {
  const result = publicProvider(provider);
  const effectiveTimeoutPolicy = resolveEffectiveTimeoutPolicy({
    providerOverrideEnabled: result.timeoutOverrideEnabled,
  ...
  return {
    ...
    timeoutPolicyRecommendation: provider.providerType === "ollama" && !result.timeoutOverrideEnabled
      ? "Local models can take several minutes to load. Consider the Local model — slow initial load preset."
      : null,
  };
}
...
export async function listAiProviders() {
  const [rows, governance] = await Promise.all([
    prisma.aiProviderConfig.findMany({ where: { deletedAt: null }, orderBy: [{ priority: "asc" }, { displayName: "asc" }], include: { health: true, _count: { select: { models: true } } } }),
    prisma.aiGovernanceSetting.findUnique({ where: { id: "global" } }),
  ]);
  return rows.map((row) => withEffectiveTimeoutPolicy(row, governance));
}
export async function getAiProvider(id: string) { const [row, governance] = await Promise.all([prisma.aiProviderConfig.findFirst({ where: { id, deletedAt: null }, include: { health: true, _count: { select: { models: true } } } }), prisma.aiGovernanceSetting.findUnique({ where: { id: "global" } })]); if (!row) throw new AiError("AI_PROVIDER_NOT_FOUND"); return withEffectiveTimeoutPolicy(row, governance); }
...
export async function deleteAiProvider(input: { providerId: string; actorUserId: string; correlationId?: string }): Promise<AiProviderDeletionResult> {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const [incomingFallbacks, preferredFeatures, fallbackFeatures, globalDefaults, models, pricing, health, metadataJobs, aiJobs] = await Promise.all([
        ...
        tx.aiJob.updateMany({ where: { providerConfigId: provider.id, status: { in: ["PENDING", "QUEUED", "RUNNING", "WAITING_RATE_LIMIT", "WAITING_PROVIDER", "RETRYING"] }, cancellationRequestedAt: null }, data: { cancellationRequestedAt: now } })
      ]);
      ...
      if (activeAudits.length) warnings.push({ code: "ACTIVE_REQUESTS_CANCELLING", message: `${activeAudits.length} in-flight AI request${activeAudits.length === 1 ? " is" : "s are"} being cancelled; no new requests can use this provider.` });
      const activeReferencesRemoved = incomingFallbacks.count + preferredFeatures.count + fallbackFeatures.count + globalDefaults.count + userPoliciesUpdated + metadataJobs.count + aiJobs.count;
    ...
    cancelAiRequestsForProvider(result.providerId);
