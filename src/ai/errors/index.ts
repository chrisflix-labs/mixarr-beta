...
export function statusForCategory(category: AiErrorCategory) {
  if (["AI_DISABLED", "FEATURE_DISABLED", "PROVIDER_DISABLED", "CAPABILITY_UNAVAILABLE", "BUDGET_EXCEEDED", "AI_PRIVACY_POLICY_BLOCKED", "AI_EXTERNAL_PROVIDER_BLOCKED", "AI_PAID_FALLBACK_BLOCKED", "AI_GLOBAL_BUDGET_EXCEEDED", "AI_PROVIDER_BUDGET_EXCEEDED", "AI_USER_BUDGET_EXCEEDED", "AI_DAILY_REQUEST_LIMIT_EXCEEDED", "AI_TOKEN_LIMIT_EXCEEDED", "AI_PROMPT_TOO_LARGE", "AI_RESPONSE_LIMIT_INVALID", "AI_MODEL_PRICING_MISSING", "AI_CONTEXT_TRIMMING_FAILED", "AI_BACKGROUND_REQUEST_BLOCKED", "AI_RETRY_COST_LIMIT_EXCEEDED", "AI_NO_ELIGIBLE_PROVIDER", "PROVIDER_CLASSIFICATION_UNKNOWN", "PAID_PROVIDER_NOT_PERMITTED", "BACKGROUND_REQUEST_NOT_PERMITTED", "MODEL_UNPRICED", "PRIVACY_MODE_INCOMPATIBLE", "DAILY_REQUEST_LIMIT_REACHED", "DAILY_COST_LIMIT_REACHED", "MONTHLY_COST_LIMIT_REACHED"].includes(category)) return 409;
  if (["PROVIDER_NOT_CONFIGURED", "PROVIDER_NOT_FOUND", "MODEL_NOT_FOUND", "MODEL_NOT_AVAILABLE"].includes(category)) return 404;
  if (["AUTHENTICATION_FAILED", "PROVIDER_UNAUTHORIZED"].includes(category)) return 401;
  if (["PERMISSION_DENIED", "PROVIDER_PERMISSION_DENIED"].includes(category)) return 403;
  if (["INVALID_REQUEST", "MODEL_NOT_CONFIGURED", "STRUCTURED_RESPONSE_INVALID", "RESPONSE_TOO_LARGE", "PROVIDER_REQUEST_INVALID", "MODEL_NOT_COMPATIBLE", "PROVIDER_ENDPOINT_INVALID"].includes(category)) return 400;
  if (["RATE_LIMITED", "PROVIDER_RATE_LIMITED", "PROVIDER_QUOTA_EXCEEDED"].includes(category)) return 429;
  if (category === "REQUEST_TIMEOUT" || category === "PROVIDER_TIMEOUT") return 504;
  if (category === "INTERNAL_AI_ERROR") return 500;
  return 502;
}
