...
Keep hidden analysis concise and prioritize the final structured result. Return only the final JSON object. Do not include Markdown fences or commentary before or after it. Keep explanations inside schema fields concise. Complete every required field and prioritize valid final JSON within the output limit.
Use schemaVersion "1.0" and camelCase keys. Unknown keys are rejected. proposedPatch may only use the supported Mixarr fields described by the user prompt. For explain, diagnose, compare_intent, suggest_names, and onboarding, use proposedPatch=null unless an explicit modification is requested. Report assumptions, conflicts, unsupported requests, side effects, and confidence. Do not claim guaranteed results.`;
