...
export async function testAiProviderConnection(providerId: string, signal?: AbortSignal, userId?: string, background = false, requestedModel?: string) {
  ...
  try {
    const result = await adapter.testConnection(config, timed.signal, selectedModel);
    ...
    await finishAdministrativeAiOperation(governanceOperation, { success: true, modelReturned: result.modelReturned, endpointMode: result.endpointMode, providerRequestId: result.providerRequestId, usage: result.usage });
