...
export async function testAiProviderConnection(providerId: string, signal?: AbortSignal) {
  ...
  try {
    ...
    let normalized = normalizeProviderError(error); if (timed.timedOut()) normalized = new AiError("REQUEST_TIMEOUT"); else if (timed.signal.aborted && ["CONNECTION_FAILED", "REQUEST_CANCELLED"].includes(normalized.category)) normalized = new AiError("REQUEST_CANCELLED"); if (normalized.category === "REQUEST_CANCELLED") throw normalized; const current = await prisma.aiProviderHealth.findUnique({ where: { providerConfigId: providerId } }); const failures = (current?.consecutiveFailureCount || 0) + 1; const next = new Date(Date.now() + Math.min(24 * 60, config.requestTimeoutMs / 1000 + 2 ** failures) * 60_000);
...
export async function refreshAiProviderModels(providerId: string, signal?: AbortSignal) {
...
export async function runDueAiHealthChecks(concurrency = 2) {
  ...
  await Promise.all(Array.from({ length: Math.min(concurrency, due.length) }, async () => { while (cursor < due.length) { const item = due[cursor++]; await testAiProviderConnection(item.id).catch(() => null); } }));
