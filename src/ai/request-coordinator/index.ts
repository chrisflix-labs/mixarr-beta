...
async function executeWithRetries<T>(input: { request: AiRequest<T>; candidate: { config: ResolvedAiProviderConfig; model: string; adapter: AiProviderAdapter }; requestId: string; auditId?: string; signal: AbortSignal; maxBytes: number; started: number; startingRetries: number; timedOut: () => boolean; timeoutMs: number; timeoutSource: string; maximumRetryAttempts: number; retryAfterPossibleBilling: boolean; estimatedAttemptCost: number; beforeRetry: (retryNumber: number, possiblePriorBilling: boolean) => Promise<unknown> }) {
  ...
  while (true) {
    ...
    try {
      ...
      const response = await input.candidate.adapter.complete(input.request, input.candidate.config, { requestId: input.requestId, providerId: input.candidate.config.id, model: input.candidate.model, signal: input.signal, maxResponseBytes: input.maxBytes, modelCapabilities: input.request.resolvedModelCapabilities });
      if (String(response.finishReason || "").toLowerCase() === "length") {
        ...
        else throw new AiError("AI_PROVIDER_TRUNCATED_RESPONSE", undefined, 422, undefined, { request_id: input.requestId, provider: input.candidate.config.displayName, model: input.candidate.model, stage: "STRUCTURED_RESPONSE", finish_reason: response.finishReason, configured_output_limit: input.request.maxOutputTokens, usage_input_tokens: response.usage?.inputTokens, usage_output_tokens: response.usage?.outputTokens, usage_total_tokens: response.usage?.totalTokens, usage_reasoning_tokens: response.usage?.reasoningTokens, usage_final_answer_tokens: response.usage?.finalAnswerTokens, usage_provider_reported: response.usage?.providerReported === true, provider_request_id: response.usage?.providerRequestId, actual_cost: response.actualCost, billing_possible: true, http_status: response.transport?.httpStatus, response_content_type: response.transport?.contentType, response_body_length: response.transport?.bodyLength });
      }
      ...
      const truncationRetry = normalized.category === "AI_PROVIDER_TRUNCATED_RESPONSE" && input.request.outputBudget?.allowTruncationRetry === true && Number(input.request.truncationRetryMaxOutputTokens || 0) > Number(input.request.maxOutputTokens || 0);
      if ((!isRetryEligible(normalized) && !truncationRetry) || retries - input.startingRetries >= retryLimit || input.signal.aborted || (mayHaveBeenBilled && !input.retryAfterPossibleBilling)) {
      ...
      if (truncationRetry) {
        input.request.maxOutputTokens = input.request.truncationRetryMaxOutputTokens;
        input.request.truncationRetry = true;
        input.request.systemInstructions = `${input.request.systemInstructions || ""}\nTruncation recovery: restart from the original inputs. Return only one complete final JSON object. Keep internal reasoning and schema-field explanations concise. Never continue or reuse partial JSON.`.trim();
        if (input.auditId) await prisma.aiRequestAudit.update({ where: { id: input.auditId }, data: { truncationRecoveryAttempted: true, configuredOutputTokenLimit: input.request.maxOutputTokens, finishReason: "length", finalContentStatus: "INCOMPLETE" } }).catch((auditError) => logAuditFailure(auditError, { requestId: input.requestId, request: input.request, provider: input.candidate.config, model: input.candidate.model }));
      }
...
export class AiRequestCoordinator {
  async complete<T = unknown>(raw: AiRequest<T>, userId?: string): Promise<AiResponse<T>> {
    ...
    request.metadataRecords = governed.preview.sanitizedMetadata; request.maxOutputTokens = governed.preview.limits.maxOutputTokens; request.resolvedModelCapabilities = governed.preview.modelCapabilities; request.truncationRetryMaxOutputTokens = governed.preview.outputBudget?.retryProviderOutputTokens; maxBytes = Math.min(maxBytes, governed.preview.responseLimits.maximumResponseBytes);
    if (auditId) await prisma.aiRequestAudit.update({ where: { id: auditId }, data: { configuredOutputTokenLimit: request.maxOutputTokens } }).catch((auditError) => logAuditFailure(auditError, { requestId, request, userId, provider: selected.primary.config, model: selected.primary.model }));
    let timeout: ReturnType<typeof resolveAiRequestTimeout>;
    try {
      timeout = resolveAiRequestTimeout({ requestTimeoutMs: request.timeoutMs, providerTimeoutMs: selected.primary.config.requestTimeoutMs, globalTimeoutMs: selected.global.defaultTimeoutMs, governanceTimeoutMs: governed.preview.timeoutPolicy.totalRequestTimeoutMs });
    } catch (error) {
    ...
    try {
      ...
      const fallbackExecution = await executeEligibleFallback(
        ...
        request.truncationRetry = false; request.maxOutputTokens = undefined; request.resolvedModelCapabilities = undefined; request.truncationRetryMaxOutputTokens = undefined;
        ...
        const fallbackReservation = await reserveAiBudget({ request, provider: explicitFallback.config, model: explicitFallback.model, userId, requestId, auditId }); reservationId = fallbackReservation.reservation.id; governed = fallbackReservation; request.maxOutputTokens = governed.preview.limits.maxOutputTokens; request.resolvedModelCapabilities = governed.preview.modelCapabilities; request.truncationRetryMaxOutputTokens = governed.preview.outputBudget?.retryProviderOutputTokens;
      ...
      if (request.responseFormat) {
        try {
          const parsed = await parseStructuredResponseWithProviderRepair({ content: response.content || "", format: request.responseFormat, maxBytes, maximumStructuredItems: governed.preview.responseLimits.maximumStructuredItems, providerRepairAttempts: request.responseFormat.allowEmbeddedJson === false ? 0 : governed.preview.structuredOutputPolicy.jsonProviderRepairAttempts, repair: async (malformedContent) => {
            ...
            try { repairResponse = await finalCandidate.adapter.complete({ ...request, systemInstructions: "Correct the supplied malformed JSON so it matches the originally requested schema. Return only the corrected JSON object. Do not add explanations, markdown, new behavior, or missing recipe behavior you cannot derive safely.", messages: [{ role: "user", content: `Malformed provider output to correct:\n${malformedContent.slice(0, 100_000)}` }], stream: false }, finalCandidate.config, { requestId, providerId: finalCandidate.config.id, model: finalCandidate.model, signal: timed.signal, maxResponseBytes: maxBytes }); }
            ...
            response.warnings.push("The provider returned malformed JSON; one bounded provider repair attempt was used.");
            console.info("[AI] structured response repair completed", { requestId, provider: finalCandidate.config.displayName, model: finalCandidate.model, elapsedMs: Date.now() - repairStarted, stage: "JSON_PROVIDER_REPAIR" });
      ...
      if (auditId && normalized.category === "AI_PROVIDER_TRUNCATED_RESPONSE") await prisma.aiRequestAudit.update({ where: { id: auditId }, data: { finishReason: "length", finalContentStatus: "INCOMPLETE", configuredOutputTokenLimit: request.maxOutputTokens, reasoningTokenCount: typeof normalized.details?.usage_reasoning_tokens === "number" ? normalized.details.usage_reasoning_tokens : undefined } }).catch((auditError) => logAuditFailure(auditError, { requestId, request, userId, provider: selected.primary.config, model: selected.primary.model }));
  ...
  async *stream<T = unknown>(raw: AiRequest<T>, userId?: string): AsyncIterable<AiStreamEvent> {
    ...
    const resolvedTimeout = resolveAiRequestTimeout({ requestTimeoutMs: request.timeoutMs, providerTimeoutMs: selected.primary.config.requestTimeoutMs, globalTimeoutMs: selected.global.defaultTimeoutMs, governanceTimeoutMs: governed.preview.timeoutPolicy.totalRequestTimeoutMs });
    const duration = Math.min(resolvedTimeout.timeoutMs, Number(process.env.AI_MAX_STREAM_DURATION_MS || 300_000)); const timed = createAiRequestSignal(request.signal, duration);
    ...
    try {
      ...
      let normalized = normalizeProviderError(error);
      if (timed.timedOut()) normalized = new AiError("AI_PROVIDER_TIMEOUT", undefined, 504, undefined, { request_id: requestId, provider: selected.primary.config.displayName, model: selected.primary.model, stage: "PROVIDER_STREAM", elapsed_ms: Date.now() - started, timeout_ms: duration, timeout_source: duration === resolvedTimeout.timeoutMs ? resolvedTimeout.timeoutSource : "stream_duration", retryable: true, usage_available: false });
