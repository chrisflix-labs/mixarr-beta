...
export class AiRequestCoordinator {
  async complete<T = unknown>(raw: AiRequest<T>, userId?: string): Promise<AiResponse<T>> {
    ...
    request.metadataRecords = governed.preview.sanitizedMetadata; request.maxOutputTokens = governed.preview.limits.maxOutputTokens; request.outputTokenLimit = governed.preview.outputTokenLimit; request.resolvedModelCapabilities = governed.preview.modelCapabilities; request.truncationRetryMaxOutputTokens = undefined; maxBytes = Math.min(maxBytes, governed.preview.responseLimits.maximumResponseBytes);
    ...
    try {
      ...
      const fallbackExecution = await executeEligibleFallback(
        ...
        request.truncationRetry = false; request.maxOutputTokens = undefined; request.outputTokenLimit = undefined; request.resolvedModelCapabilities = undefined; request.truncationRetryMaxOutputTokens = undefined;
        ...
        const fallbackReservation = await reserveAiBudget({ request, provider: explicitFallback.config, model: explicitFallback.model, userId, requestId, auditId }); reservationId = fallbackReservation.reservation.id; governed = fallbackReservation; request.maxOutputTokens = governed.preview.limits.maxOutputTokens; request.outputTokenLimit = governed.preview.outputTokenLimit; request.resolvedModelCapabilities = governed.preview.modelCapabilities; request.truncationRetryMaxOutputTokens = undefined;
      ...
      if (request.responseFormat) {
        try {
          ...
          throw new AiError(request.featureKey === "recipe_copilot" ? "AI_FEATURE_INVALID_STRUCTURED_OUTPUT" : "AI_PROVIDER_INVALID_STRUCTURED_RESPONSE", undefined, 422, undefined, { ...structured.details, ...usageDetails, request_id: requestId, provider: finalCandidate.config.displayName, model: finalCandidate.model, stage: structured.details?.failure_stage || "STRUCTURED_RESPONSE", finish_reason: response.finishReason, elapsed_ms: Date.now() - started });
        }
      }
  ...
  async *stream<T = unknown>(raw: AiRequest<T>, userId?: string): AsyncIterable<AiStreamEvent> {
    ...
    request.metadataRecords = governed.preview.sanitizedMetadata; request.maxOutputTokens = governed.preview.limits.maxOutputTokens; request.outputTokenLimit = governed.preview.outputTokenLimit; maxBytes = Math.min(maxBytes, governed.preview.responseLimits.maximumResponseBytes);
