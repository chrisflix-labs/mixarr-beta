...
import { alertDeduplicationKey, applyMetadataPrivacyPolicy, budgetPeriod, DEFAULT_METADATA_ALLOWLIST, estimateRequestCost, estimateTokens, evaluateCostLimit, evaluateRetryCost, resolveEffectiveTokenLimits, strictestPrivacyMode, trimContext, validatePromptLimits, wouldExceedBudget, type AiPrivacyMode } from "./policy";
...
import { calculateOutputBudget } from "./outputBudget";
import { resolveModelCapabilities } from "../registry/modelCapabilities";
import { resolveNormalizedOutputTokenLimit } from "./outputTokenLimits";
...
export async function updateAiGovernanceSettings(raw: unknown, actorId: string) {
  ...
  if (input.totalRequestTimeoutMs != null && input.totalRequestTimeoutMs < 30_000) throw new AiError("INVALID_REQUEST", "Total AI request timeout must be between 30 and 600 seconds.", 400);
...
export async function previewAiRequest(input: PreviewContext) {
  const [governance, providerRow, modelLimit, pricing, userLimit, feature, acknowledgment] = await Promise.all([
    ...
    prisma.aiProviderModel.findUnique({ where: { providerConfigId_modelIdentifier: { providerConfigId: input.provider.id, modelIdentifier: input.model } }, select: { contextSize: true, maximumInputTokens: true, maximumOutputTokens: true, maximumCombinedTokens: true, modelCategory: true, structuredOutput: true, jsonMode: true, capabilityMetadata: true } }),
  ...
  const providerOutputLimit = providerRow.governanceBudget?.maximumOutputTokens ?? providerRow.maximumOutputTokens;
  const featureTokenLimits = administrativeInferenceTest ? {} : featureConfig.tokenLimits || {};
  const limitScopes = [{ maximumInputTokens: governance.maximumInputTokens, maximumOutputTokens: governance.maximumOutputTokens, maximumCombinedTokens: governance.maximumCombinedTokens }, { maximumInputTokens: providerRow.governanceBudget?.maximumInputTokens ?? providerRow.maximumContextTokens, maximumOutputTokens: providerOutputLimit, maximumCombinedTokens: providerRow.governanceBudget?.maximumCombinedTokens }, { maximumInputTokens: modelLimit?.maximumInputTokens ?? modelLimit?.contextSize, maximumOutputTokens: modelLimit?.maximumOutputTokens, maximumCombinedTokens: modelLimit?.maximumCombinedTokens }, { maximumInputTokens: userLimit?.maximumInputTokens, maximumOutputTokens: userLimit?.maximumOutputTokens, maximumCombinedTokens: userLimit?.maximumCombinedTokens }, featureTokenLimits];
  ...
  const effectiveWithoutRequest = resolveEffectiveTokenLimits(limitScopes);
  const estimatedInput = estimateTokens(finalText);
  const availableOutputTokens = Math.max(0, Math.min(effectiveWithoutRequest.maximumOutputTokens, effectiveWithoutRequest.maximumCombinedTokens - estimatedInput, modelCapabilities.maximumOutputTokens ?? Number.MAX_SAFE_INTEGER));
  const outputBudget = input.request.outputBudget ? calculateOutputBudget({ policy: input.request.outputBudget, capabilities: modelCapabilities, availableOutputTokens }) : undefined;
  const requestedOutputTokens = outputBudget ? input.request.truncationRetry ? Math.min(outputBudget.retryProviderOutputTokens, Math.max(outputBudget.providerOutputTokens, Number(input.request.maxOutputTokens || 0))) : outputBudget.providerOutputTokens : input.request.maxOutputTokens;
  const configuredModelLimits = [modelLimit?.maximumOutputTokens, modelCapabilities.maximumOutputTokens].filter((value): value is number => value != null && value > 0);
  const outputTokenLimit = resolveNormalizedOutputTokenLimit({ requestedOutputTokens, configuredGlobalLimit: governance.maximumOutputTokens, configuredProviderLimit: providerOutputLimit, configuredFeatureLimit: administrativeInferenceTest ? null : featureTokenLimits.maximumOutputTokens, configuredUserLimit: userLimit?.maximumOutputTokens, modelOutputLimit: configuredModelLimits.length ? Math.min(...configuredModelLimits) : null, defaultOutputTokens: modelCapabilities.defaultOutputTokens });
  const limits = validatePromptLimits({ text: finalText, messageCount: sections.length, metadataRecordCount: metadata.payload.length, limits: { maximumPromptCharacters: governance.maximumPromptCharacters, maximumPromptBytes: governance.maximumPromptBytes, maximumContextMessages: governance.maximumContextMessages, maximumMetadataRecords: governance.maximumMetadataRecords, ...limitScopes[0] }, requestedOutputTokens: outputTokenLimit.effectiveOutputTokens, tokenScopes: limitScopes.slice(1) });
  if (limits.maxOutputTokens < outputTokenLimit.effectiveOutputTokens) { outputTokenLimit.effectiveOutputTokens = limits.maxOutputTokens; outputTokenLimit.limitingSource = "combined"; }
  if (administrativeInferenceTest && outputTokenLimit.effectiveOutputTokens < 128) throw new AiError("AI_PROVIDER_TEST_TOKEN_LIMIT_TOO_LOW", undefined, 409, undefined, { requested_max_tokens: 128, effective_max_tokens: outputTokenLimit.effectiveOutputTokens, output_token_limiting_source: outputTokenLimit.limitingSource, configured_value: outputTokenLimit.effectiveOutputTokens, token_limit: outputTokenLimit });
  ...
  if (outputBudget && outputBudget.availableOutputTokens < outputBudget.recommendedMinimum) {
    const estimatedMaximumCost = localZeroCost ? 0 : usablePricing ? estimateRequestCost({ inputTokens: limits.estimatedInputTokens, maximumOutputTokens: outputBudget.recommendedMinimum, pricing: { ...usablePricing, inputPricePerMillion: usablePricing.inputPricePerMillion?.toString(), outputPricePerMillion: usablePricing.outputPricePerMillion?.toString(), cachedInputPricePerMillion: usablePricing.cachedInputPricePerMillion?.toString(), reasoningPricePerMillion: usablePricing.reasoningPricePerMillion?.toString(), fixedRequestCost: usablePricing.fixedRequestCost?.toString() } }).maximumEstimatedCost : undefined;
    throw new AiError("AI_REQUIRED_OUTPUT_BUDGET_EXCEEDS_LIMIT", undefined, 409, undefined, { configured_output_token_limit: outputBudget.availableOutputTokens, recommended_minimum: outputBudget.recommendedMinimum, requested_final_answer_tokens: outputBudget.requestedFinalAnswerTokens, reasoning_reserve: outputBudget.reasoningReserve, estimated_maximum_cost: estimatedMaximumCost, currency: governance.currency, applicable_governance_limit: effectiveWithoutRequest.maximumOutputTokens, model_maximum_output_tokens: modelCapabilities.maximumOutputTokens });
  }
...
export async function beginAdministrativeAiOperation(input: { provider: ResolvedAiProviderConfig; source: "CONNECTION_TEST" | "MODEL_DISCOVERY"; model?: string; userId?: string; signal?: AbortSignal; background?: boolean }) {
  ...
  if (input.source === "CONNECTION_TEST" || pricing) {
    try {
      const governed = await reserveAiBudget({ request: { featureKey, systemInstructions: input.source === "CONNECTION_TEST" ? "You are performing an AI provider connectivity test. Return only valid JSON and no additional text." : undefined, messages: [{ role: "user", content: input.source === "CONNECTION_TEST" ? 'Return exactly this JSON object: {"ok":true}' : "Mixarr model discovery." }], maxOutputTokens: input.source === "CONNECTION_TEST" ? 128 : 1, thinkingMode: input.source === "CONNECTION_TEST" ? "disabled" : undefined, requestSource: input.source, signal: input.signal }, provider: input.provider, model: specialModel, userId: input.userId, requestId, auditId });
      reservationId = governed.reservation.id; resolvedPolicy = governed.preview.policyDecision;
      if (input.source === "CONNECTION_TEST") (resolvedPolicy as any).__providerTest = { outputTokenLimit: governed.preview.outputTokenLimit, maximumEstimatedCost: governed.preview.cost.maximumEstimatedCost };
      if (input.source === "CONNECTION_TEST" && auditId) await prisma.aiRequestAudit.update({ where: { id: auditId }, data: { requestedOutputTokenLimit: governed.preview.outputTokenLimit.requestedOutputTokens, effectiveOutputTokenLimit: governed.preview.outputTokenLimit.effectiveOutputTokens, configuredOutputTokenLimit: governed.preview.outputTokenLimit.effectiveOutputTokens, outputTokenLimitingSource: governed.preview.outputTokenLimit.limitingSource, thinkingModeRequested: "disabled" } });
    } catch (error) {
