...
export async function beginAdministrativeAiOperation(input: { provider: ResolvedAiProviderConfig; source: "CONNECTION_TEST" | "MODEL_DISCOVERY"; model?: string; userId?: string; signal?: AbortSignal; background?: boolean }) {
  ...
  if (input.source === "CONNECTION_TEST" || pricing) {
    try {
      const governed = await reserveAiBudget({ request: { featureKey, messages: [{ role: "user", content: input.source === "CONNECTION_TEST" ? "Mixarr connection test." : "Mixarr model discovery." }], maxOutputTokens: input.source === "CONNECTION_TEST" ? 16 : 1, requestSource: input.source, signal: input.signal }, provider: input.provider, model: specialModel, userId: input.userId, requestId, auditId });
