...
export async function approveRecipe(userId: string, recipeId: string, input: { mode?: RecipeImportMode; grantedPermissions?: string[]; confirmConsequences?: string[] }) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const updated = await tx.playlistRecipe.update({ where: { id: recipe.id }, data: { approvalState, quarantineState: "NONE", quarantineReason: null, enabled: true, automationPolicyJson: json(automationPolicy), refreshPolicyJson: json(refreshPolicy), grantedPermissionsJson: json(desired), restrictedPermissionsJson: json(restricted), approvedById: userId, approvedAt: new Date(), revokedLocallyAt: null } });
    await writeRecipeAudit({ recipeId, recipeVersion: updated.recipeVersion, eventType: restricted.length ? "RECIPE_APPROVED_WITH_RESTRICTIONS" : "RECIPE_APPROVED", actorId: userId, correlationId, description: restricted.length ? `Recipe approved with ${restricted.length} restricted permission(s).` : "Recipe approved after local review.", previousState: { approvalState: recipe.approvalState, quarantineState: recipe.quarantineState }, newState: { approvalState, quarantineState: "NONE", mode }, permissions: { granted: desired, restricted }, trustState: updated.trustState, riskLevel: updated.riskLevel }, tx);
    return updated;
  });
