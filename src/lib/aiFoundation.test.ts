...
describe("v2.4.0 persistence, API, UI, and safety contracts", () => {
  ...
  it("exposes accessible responsive states and only the reviewed v2.4.2 feature is usable", () => { const ui = read("src/components/AiProviderDashboard.tsx"); const css = read("src/components/AiProviderDashboard.module.css"); const features = read("src/ai/features/registry.ts"); assert.match(ui, /aria-live="polite"/); assert.match(ui, /Cancel test/); assert.match(ui, /AI is disabled by default|AI disabled/); assert.match(css, /:focus-visible/); assert.match(css, /@media \(max-width: 480px\)/); assert.match(features, /natural_language_playlist_requests[\s\S]*implemented: true/); assert.equal((features.match(/implemented: true/g) || []).length, 1); });
  it("documents encryption backup, Docker host networking, ChatGPT limitations, SSL, and the no-mutation boundary", () => { const docs = read("docs/AI_PROVIDER_FOUNDATION_V240.md"); for (const marker of [/host\.docker\.internal/, /Back it up separately/i, /ChatGPT Subscription/, /certificate verification/i, /cannot add, remove, unlock, create, regenerate/i]) assert.match(docs, marker); assert.equal(JSON.parse(read("package.json")).version, "2.4.3"); });
});
