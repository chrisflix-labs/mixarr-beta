...
before(async () => {
  server = http.createServer(async (request, response) => {
    ...
    if (request.url === "/v1/chat/completions") return response.end(JSON.stringify({ model: body.model, choices: [{ message: { content: body.response_format ? '{"ok":true}' : "OK" }, finish_reason: "stop" }], usage: { prompt_tokens: 2, completion_tokens: 1, total_tokens: 3 } }));
    ...
    if (request.url === "/api/chat") return response.end(JSON.stringify({ model: body.model, message: { content: "OK" }, done: true, prompt_eval_count: 2, eval_count: 1 }));
...
describe("provider adapter contract", () => {
  it("discovers models, tests chat completion, normalizes usage, and supports structured output", async () => { const adapter = new OpenAiCompatibleAdapter("openai_compatible"); const provider = config("openai_compatible", "/v1"); const models = await adapter.discoverModels(provider); assert.equal(models[0].id, "fast-model"); const test = await adapter.testConnection(provider); assert.equal(test.connected, true); const response = await adapter.complete({ featureKey: "contract", messages: [{ role: "user", content: "fixed" }], responseFormat: { type: "json", name: "test", schema: z.object({ ok: z.boolean() }) } }, provider, { requestId: "request", providerId: provider.id, model: "fast-model", signal: new AbortController().signal, maxResponseBytes: 4096 }); assert.equal(response.usage?.totalTokens, 3); assert.equal(response.content, '{"ok":true}'); });
  ...
  it("passes Ollama discovery, connection, and completion contracts", async () => { const adapter = new OllamaAdapter(); const provider = config("ollama"); assert.equal((await adapter.discoverModels(provider))[0].id, "llama-test"); assert.equal((await adapter.testConnection(provider)).connected, true); const response = await adapter.complete({ featureKey: "contract", messages: [{ role: "user", content: "fixed" }] }, provider, { requestId: "ollama", providerId: provider.id, model: "llama-test", signal: new AbortController().signal, maxResponseBytes: 4096 }); assert.equal(response.content, "OK"); assert.match(response.warnings[0], /cost not tracked/i); });
...
describe("v2.4.0 persistence, API, UI, and safety contracts", () => {
  ...
  it("exposes accessible responsive states and only reviewed user-facing features are usable", () => { const ui = read("src/components/AiProviderDashboard.tsx"); const css = read("src/components/AiProviderDashboard.module.css"); const features = read("src/ai/features/registry.ts"); assert.match(ui, /aria-live="polite"/); assert.match(ui, /Cancel test/); assert.match(ui, /AI is disabled by default|AI disabled/); assert.match(css, /:focus-visible/); assert.match(css, /@media \(max-width: 480px\)/); for (const key of ["natural_language_playlist_requests", "recipe_copilot", "playlist_ai_summaries", "metadata_suggestions"]) assert.match(features, new RegExp(`${key}[\\s\\S]*implemented: true`)); assert.equal((features.match(/implemented: true/g) || []).length, 4); });
  it("documents encryption backup, Docker host networking, ChatGPT limitations, SSL, and the no-mutation boundary", () => { const docs = read("docs/AI_PROVIDER_FOUNDATION_V240.md"); for (const marker of [/host\.docker\.internal/, /Back it up separately/i, /ChatGPT Subscription/, /certificate verification/i, /cannot add, remove, unlock, create, regenerate/i]) assert.match(docs, marker); assert.equal(JSON.parse(read("package.json")).version, "2.4.7"); });
});
