...
describe("personalization roadmap", () => {
  it("marks v2.1.x complete and keeps v2.2.x current", () => {
    ...
    assert.equal(roadmapCycles.find((cycle) => cycle.id === "2.2.x")?.status, "current");
    assert.equal(currentRoadmapRelease()?.version, "2.2.8");
  });
});
