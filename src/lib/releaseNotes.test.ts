...
describe("release notes", () => {
  it("sorts release notes from oldest to newest", () => {
    ...
    ordered.pop(); // v2.2.8 through v2.0.5 are asserted separately below.
    ordered.pop();
    ordered.pop();
    ordered.pop();
    ordered.pop();
  ...
  it("sorts release notes from newest to oldest", () => {
    ...
    ordered.shift(); // v2.2.8 through v2.0.5 are asserted separately below.
    ordered.shift();
    ordered.shift();
    ordered.shift();
    ordered.shift();
  ...
  it("adds the v2.2.8 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "2.2.8");
    assert.equal(latest.title, "Playlist Health Monitoring & Alerts");
