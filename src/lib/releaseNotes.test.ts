...
describe("release notes", () => {
  ...
  it("adds the v2.0.2 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "2.0.2");
    assert.equal(latest.title, "Recommendation Tuning");
    assert.deepEqual(latest.badges, ["Playlists", "Smart Builder", "BPM", "Popularity", "UI"]);
  });
