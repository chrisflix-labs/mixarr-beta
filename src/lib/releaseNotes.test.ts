...
describe("release notes", () => {
  ...
  it("adds the v1.5.0 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "1.5.0");
    assert.equal(latest.title, "Beta Feature Flags & Experimental Access");
    assert.deepEqual(latest.badges, ["Beta", "Settings", "Dashboard", "Preview", "Support"]);
  });
