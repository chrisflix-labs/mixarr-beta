...
describe("release notes", () => {
  ...
  it("adds the v2.0.0 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "2.0.0");
    assert.equal(latest.title, "Smart Mix Engine v2 Foundation");
    assert.deepEqual(latest.badges, ["Beta", "Smart Builder", "Playlists", "BPM", "History"]);
  });
