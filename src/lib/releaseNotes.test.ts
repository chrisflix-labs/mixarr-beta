...
describe("release notes", () => {
  ...
  it("places v1.3.9 above v1.3.8 without string sorting", () => {
    const ordered = getReleaseNotesNewestFirst([
    ...
    assert.deepEqual(ordered.map((note) => note.version), ["v2.0.0", "v1.3.10", "v1.3.9", "v1.3.8"]);
  });

  it("adds the v1.3.9 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "1.3.9");
    assert.equal(latest.title, "v2.0.0 Readiness & Beta Hardening");
    assert.deepEqual(latest.badges, ["Beta", "Readiness", "Diagnostics", "Settings", "Release Notes", "Roadmap"]);
  });
