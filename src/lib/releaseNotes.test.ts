...
describe("release notes", () => {
  it("sorts release notes from oldest to newest", () => {
    ...
    ordered.pop(); // v2.4.21 through v2.0.5 are asserted separately below.
    ordered.pop();
    ordered.pop();
    ordered.pop();
    ordered.pop();
  ...
  it("sorts release notes from newest to oldest", () => {
    ...
    ordered.shift(); // v2.4.21 through v2.0.5 are asserted separately below.
    ordered.shift();
    ordered.shift();
    ordered.shift();
    ordered.shift();
  ...
  it("uses semantic version ordering", () => {
    ...
    assert.equal(compareSemanticVersions("v1.3.7.1", "v1.3.7") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.7.2", "v1.3.7.1") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.8", "v1.3.7.2") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.9", "v1.3.8") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.9.1", "v1.3.9") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.9.2", "v1.3.9.1") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.10", "v1.3.9.2") > 0, true);
    assert.equal(compareSemanticVersions("v1.5.0", "v1.3.10") > 0, true);
    assert.equal(compareSemanticVersions("v1.3.10", "v1.3.9") > 0, true);
    assert.equal(compareSemanticVersions("v2.0.0", "v1.3.10") > 0, true);
  ...
  it("keeps the beta Discord invite configurable", () => {
    assert.equal(typeof MIXARR_BETA_DISCORD_URL, "string");
  });
  ...
  it("places v1.3.7.2 above v1.3.7.1 while keeping v1.3.8 newer", () => {
    const ordered = getReleaseNotesNewestFirst([
      { version: "v1.3.7", title: "Plex Matching & Track Sync Polish", badges: ["Plex", "Library Sync"], changes: ["Plex"] },
      { version: "v1.3.7.1", title: "Remove Healthy Tracks Card", badges: ["Hotfix"], changes: ["Hotfix"] },
      { version: "v1.3.7.2", title: "Dashboard Card Refresh Fix", badges: ["Hotfix"], changes: ["Hotfix"] },
      { version: "v1.3.8", title: "Later", badges: ["Beta"], changes: ["Later"] },
    ]);

    assert.deepEqual(ordered.map((note) => note.version), ["v1.3.8", "v1.3.7.2", "v1.3.7.1", "v1.3.7"]);
  });

  it("places v1.3.9.2 above v1.3.9.1 while keeping later patches and majors newer", () => {
    const ordered = getReleaseNotesNewestFirst([
      { version: "v1.3.10", title: "Future patch", badges: ["Beta"], changes: ["Future"] },
      { version: "v1.3.8", title: "Previous", badges: ["Beta"], changes: ["Previous"] },
      { version: "v2.0.0", title: "Major", badges: ["Beta"], changes: ["Major"] },
      { version: "v1.3.9", title: "Current", badges: ["Beta"], changes: ["Current"] },
      { version: "v1.3.9.1", title: "Hotfix", badges: ["Hotfix"], changes: ["Hotfix"] },
      { version: "v1.3.9.2", title: "Second hotfix", badges: ["Hotfix"], changes: ["Hotfix"] },
    ]);

    assert.deepEqual(ordered.map((note) => note.version), ["v2.0.0", "v1.3.10", "v1.3.9.2", "v1.3.9.1", "v1.3.9", "v1.3.8"]);
  });

  it("adds the v2.4.21 release note at the top", () => {
    const [latest] = getReleaseNotesNewestFirst();

    assert.equal(latest.version, "2.4.21");
    assert.equal(latest.title, "Lossless Library Intelligence Backup & Restore");
    assert.ok(latest.badges.includes("Backup"));
  });
  ...
  it("links the sidebar navigation to the beta support page", () => {
    const sidebarPath = join(process.cwd(), "src", "components", "Sidebar.tsx");
    const sidebar = readFileSync(sidebarPath, "utf8");

    assert.match(sidebar, /href: "\/support"/);
    assert.match(sidebar, /Beta Support/);
  });
